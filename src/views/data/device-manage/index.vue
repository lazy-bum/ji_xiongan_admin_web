<template>
  ...
  >
    ...
    <template #toolbar>
      <a-button
        type="primary"
        :disabled="!$auth('data:device-manage:create')"
        @click="openMenuModal({})"
      >
        新增
      </a-button>
...
<script lang="ts" setup>
    ...
    const [formRef] = await showModal({
      modalProps: {
        ...
        onFinish: async (values) => {
          ...
          const params = { ...values };
          if (record.id) {
            await Api.dataDeviceManage.deviceManageUpdate({ id: record.id }, params);
          } else {
            await Api.dataDeviceManage.deviceManageCreate(params);
          }
  ...
  const delRowConfirm = async (record: TableListItem) => {
    await Api.dataDeviceManage.deviceManageDelete({ id: record.id });
    dynamicTableInstance?.reload();
  };
      ...
      actions: ({ record }) => [
        {
          label: '编辑',
          auth: {
            perm: 'data:device-manage:update',
            effect: 'disable',
          },
          onClick: () => {
        ...
        {
          label: '删除',
          auth: 'data:device-manage:delete',
          popConfirm: {
            title: '你确定要删除吗？',
            placement: 'left',
