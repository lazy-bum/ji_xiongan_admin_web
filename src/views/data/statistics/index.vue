<template>
  <DynamicTable
    row-key="id"
    header-title="总指标"
    :data-request="Api.dataStatistics.statisticsList"
    :columns="columns"
    :search="false"
    bordered
    size="small"
  ...
  >
    ...
    <template #toolbar>
      <a-button
        type="primary"
        :disabled="!$auth('data:statistics:create')"
        @click="openMenuModal({})"
      >
        新增
      </a-button>
...
<script lang="ts" setup>
    ...
    const [formRef] = await showModal({
      modalProps: {
        ...
        onFinish: async (values) => {
          ...
          const params = { ...values };
          if (record.id) {
            await Api.dataStatistics.statisticsUpdate({ id: record.id }, params);
          } else {
            await Api.dataStatistics.statisticsCreate(params);
          }
  ...
  const delRowConfirm = async (record: TableListItem) => {
    await Api.dataStatistics.statisticsDelete({ id: record.id });
    dynamicTableInstance?.reload();
  };
